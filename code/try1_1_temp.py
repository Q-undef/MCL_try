...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # 尝试用where替换（失败，因为条件必须是long，不能是float
            # replacement = torch.tensor(255).to(device)
            # masked_img = torch.where(mask_unlabeled_volume_batch == 0, replacement, mask_unlabeled_volume_batch)
