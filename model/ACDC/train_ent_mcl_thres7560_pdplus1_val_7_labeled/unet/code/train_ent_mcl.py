...
parser.add_argument('--exp', type=str,
                    default='ACDC/train_ent_mcl', help='experiment_name')
...
def random_mask_zero(data, patch_size, image_size=256, mask_ratio=0.75):       
    ...
    # s是patch总个数
    s = [x for x in range(patch_num*patch_num)] # 0~255 
    random.shuffle(s)
    ...
    # 逐元素相乘
    new_data = new_data * mask  # [12, 1, 256, 256]
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # student前向传播并计算softmax，以概率形式表示输出
            outputs = model(volume_batch)
            outputs_soft = torch.softmax(outputs, dim=1)
            
            # teacher输出，no_grad表示不会进行参数梯度更新
            with torch.no_grad():
            ...
            # 被选中的像素为1，否则为0。参与后续weight计算。
            ent_mask = (EMap >= threshold).float()  # [12, 256, 256]
            ent_mask = torch.unsqueeze(ent_mask, 1) # [12, 1, 256, 256]

            # 给原始图像盖上mask
            mask_unlabeled_volume_batch = ent_mask_zero(unlabeled_volume_batch, EMap, threshold)  # [12, 1, 256, 256]

            # student
            logits_masked_unlabeled_volume = model(mask_unlabeled_volume_batch)  # [12, 4, 256, 256]

            # teacher生成的伪标签，通过选择每个像素最高概率的类别得到的
            pseudo_label = torch.argmax(torch.softmax(ema_output, dim=1), dim=1).squeeze(0) # [12, 256, 256]
            ...
            loss_ce_2_mask_1 = loss_ce_2_mask_1.sum() / (ent_mask_1.sum()+1.0)

            loss_ce_2_mask_2 = loss_ce_2 * ent_mask_2.float()
            loss_ce_2_mask_2 = loss_ce_2_mask_2.sum() / (ent_mask_2.sum()+1.0)
            ...
            # 每200个循环计算一次验证数据集评估模型性能。如果当前性能优于最佳，则保存模型。
            if iter_num > 0 and iter_num % 200 == 0:
                # 将模型设置为评估模式,模型的行为会有所不同，比如启用Dropout，BatchNormalization等
                model.eval()
